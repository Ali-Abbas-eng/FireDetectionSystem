"""
This file contains three classes for building a U-Net model using TensorFlow.

The `DownscaleBlock` class represents a single down-sampling block of a convolutional neural network. It applies two
convolutional filters, ReLU activation, and batch normalization to the input tensor followed by a max pooling operation.

The `UpscaleBlock` class represents an upscale block for use in an image super-resolution model. The block consists
of an upsampling layer, a concatenation layer, two convolutional layers, two leaky ReLU activation layers,
and two batch normalization layers.

The `UNet` class represents a U-Net model for image segmentation. It takes as input the path to the directory where
the trained model will be saved and builds the U-Net model using the `DownscaleBlock` and `UpscaleBlock` classes.
"""
...
class UNet(tf.keras.Model):
    ...
    def call(self, x, *args, **kwargs):
        ...
        return self.activation(self.conv_layer(u4))


def get_fire_segmentation_model(model_directory: str = os.path.join('fire_detection', 'models'),
                                file_name: str = 'checkpoint.h5',
